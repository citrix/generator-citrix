...
namespace <%= appName %>
{
    ...
    {
      async static Task ListVersion()
        {
            //create the URI for Netscaler login
            string nsURI = string.Format(@"http://{0}:{1}/nitro/v1/config/nsversion",NSAddress,NSPort);
            ...
            Console.WriteLine("Return value from Netscaler CPX version API");
